...
The driving workload is the Unreal Engine source control plugin (`TheThreeThousands/UEGitPlugin`, branch `dev-3k`). Three properties of it set every requirement here:
...
So the steady state for a studio of forty artists is one cursor walk per `ListTtl` plus one single-page probe per artist per `AdmissionTtl`, against tens of round trips per artist per 30 seconds today.
...
Two deployments sharing `TokenKeys` and `PublicBaseUrl` are interchangeable from a client's point of view, which is what allows one hostname resolved by split-horizon DNS to serve an on-premises full cache and a forge-adjacent metadata instance without a client needing two credential entries.
