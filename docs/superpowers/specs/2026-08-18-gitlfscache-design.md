...
**One project serving as both a dotnet tool and a container.** `ktsu.Sdk.Tool` deliberately clears `RuntimeIdentifiers`, because under `PackAsTool` the .NET SDK turns each entry into a separate runtime-specific tool package and seven packages then race over one intermediate output directory. Container publishing needs a runtime identifier, supplied through the separate `ContainerRuntimeIdentifiers` property, so the two should not collide. This has to be verified in the first implementation step. If `dotnet pack` and the `PublishContainer` target do fight, the fallback is a second thin host project for the container, sharing the same `Program.cs` by file link so the two entry points still cannot drift.
