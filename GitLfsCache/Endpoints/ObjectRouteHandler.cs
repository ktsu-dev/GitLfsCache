...
/// <summary>
/// Handles the object routes: batch, transfer and verify.
/// </summary>
/// <remarks>
/// These three stay together because they genuinely share their front half — validate the transfer
/// token and decide whether the bytes come from the store or from upstream — which is the argument
/// the original single handler was built on. What they never shared was the lock subsystem, which now
/// lives in <see cref="LockRouteHandler"/>.
/// </remarks>
...
internal sealed class ObjectRouteHandler(
	...
	ILogger<ObjectRouteHandler> logger)
{
	...
	/// <summary>
	/// Serves an object, from the store when it is there and from upstream when it is not.
	/// </summary>
	/// <param name="context">The request context.</param>
	/// <param name="route">The parsed route.</param>
	/// <param name="cancellationToken">Cancels the transfer.</param>
	/// <returns>A task that completes when the response has been written.</returns>
	public async Task DownloadAsync(HttpContext context, LfsRoute route, CancellationToken cancellationToken)
	{
		Ensure.NotNull(context);
		Ensure.NotNull(route);
	...
	/// <returns>A task that completes when the response has been written.</returns>
	public async Task UploadAsync(HttpContext context, LfsRoute route, CancellationToken cancellationToken)
	{
		Ensure.NotNull(context);
		Ensure.NotNull(route);
	...
	/// <summary>
	/// Passes an upload verification upstream, which is the authority on whether it succeeded.
	/// </summary>
	/// <param name="context">The request context.</param>
	/// <param name="route">The parsed route.</param>
	/// <param name="cancellationToken">Cancels the verification.</param>
	/// <returns>A task that completes when the response has been written.</returns>
	public async Task VerifyAsync(HttpContext context, LfsRoute route, CancellationToken cancellationToken)
	{
		Ensure.NotNull(context);
		Ensure.NotNull(route);
